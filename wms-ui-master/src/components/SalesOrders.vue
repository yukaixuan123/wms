<template>
  <div id="kj">
    ...
    <transition-group name="lyric">
      <div id="form" v-if="showDefaultAttr" key="1">
        <el-form :inline="true" :model="formInline" class="demo-form-inline">
          <el-form-item label="销售单号" id="form_top">
            <el-input
          ...
          <el-form-item label="销售日期" prop="startDate">
            <el-date-picker
    ...
    >
      ...
      <el-table-column fixed label="销售日期" width="170">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" fixed="right" width="100">
        <template slot-scope="scope">
          <span v-if="scope.row.sale_state == 2"></span>
          <span v-if="scope.row.sale_state == 1 || scope.row.sale_state == 0">
            <img
              ...
              @click="handleEdit(scope.row.sale_code)"
            />
            <img
              ...
              width="28px"
              height="28px"
              @click="handleDelete(scope.row.sale_codeId)"
            />
          </span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    qd() {
      $http
        .put(
          "http://localhost:5149/api/Wh_sale_h/Wh_sale_h__sale_state?sale_state=" +
            this.resource +
            "&sure_man=" +
      ...
      if (zt == 0) {
        ...
        this.$confirm("是否撤销所选单据", "提示", {
          ...
          .then(() => {
            $http
              .put(
                "http://localhost:5149/api/Wh_sale_h/Wh_sale_h__chexiao?sure_man=" +
                  localStorage.getItem("name") +
                  "&sale_codeId=" +
                  this.buy_codeId +
                  ""
    ...
    inserok() {
      this.$router.push("/SalesOrdersAdd");
    },
    //查询方法
    find() {
      var t = this;
    ...
    search() {
      ...
      if (this.formInline.bh != "") {
        str += "and sale_code like '%" + this.formInline.bh + "%'";
      }
      if (this.formInline.zt != "") {
        if (this.formInline.zt == 2) {
    ...
    handleSelectionChange(val) {
      ...
      for (let i = 0; i < this.multipleSelection.length; i++) {
        this.buy_codeId = this.multipleSelection[i].sale_codeId;
      }
    ...
    handleDelete(buy_codeId) {
      this.$confirm("此操作将永久删除该信息, 是否继续?", "提示", {
        ...
        .then(() => {
          $http
            .put(
              "http://localhost:5149/api/Wh_sale_h/Wh_sale_h_Delete?sale_codeId=" +
                buy_codeId +
                ""
            )
            .then((res) => {
    ...
    delAll() {
      this.$confirm("此操作将永久删除该信息, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          var strs = [];
          for (let i = 0; i < this.multipleSelection.length; i++) {
            strs.push(this.multipleSelection[i].sale_codeId);
          }
          console.log(strs);
          $http
            .put(
              "http://localhost:5149/api/Wh_sale_h/Wh_sale_h_DeleteArry/route",
              strs
            )
            .then((res) => {
              if (res) {
