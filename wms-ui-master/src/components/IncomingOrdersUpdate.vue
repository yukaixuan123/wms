  ...
  <div>
    ...
    <div id="adds">
      ...
      <div style="margin-top: 30px">
        ...
        >
          <el-form-item label="进货单号" id="yyy" prop="buy_code">
            <el-input
              disabled
          ...
          <el-form-item label="供应商" id="yyy" prop="supply_code">
            <el-select
          ...
          <el-form-item label="进货日期" id="yyy" prop="buy_date">
            <el-date-picker
              v-model="formInline.buy_date"
              type="datetime"
              id="tex"
              placeholder="选择进货时间"
    ...
    <div id="addx">
      <div
        ...
        "
      >
        <h4 style="margin-left: 25px">进货明细</h4>
      </div>
      <div style="margin-top: 50px; margin-left: 30px">
        ...
        >
          ...
          <el-table-column prop="item_inventory" label="当前库存" width="90">
          </el-table-column>
          <el-table-column label="货品数量" width="115px">
            <template slot-scope="scope">
              <input
                ...
                v-model="tableData[scope.$index].buy_num"
                value="0"
              />
            </template>
          </el-table-column>
          <el-table-column label="进货单价" width="115px">
            <template slot-scope="scope">
              <input
                type="text"
                style="width: 80px; height: 25px; border: 1px solid #eeeeee"
                ...
                v-on:keyup="onKeyPress(scope.$index + 1)"
                v-model="tableData[scope.$index].buy_goods_money"
              />
            </template>
          </el-table-column>
          <el-table-column label="金额" width="85">
            <template slot-scope="scope">
              <input
                ...
                :value="scope.row.buy_num * scope.row.buy_goods_money"
              />
            </template>
          </el-table-column>
          <el-table-column prop="create_by" label="备注" width="123px">
            <template slot-scope="scope">
              <input
...
export default {
  data() {
    return {
      ...
      rules: {
        buy_code: [{ required: true, message: "请填写单号!", trigger: "blur" }],
        supply_code: [
          { required: true, message: "请填写供应商!", trigger: "change" },
        ],
        buy_date: [{ required: true, message: "请填写日期!", trigger: "blur" }],
      },
    };
  },
  methods: {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.tableData = this.tableData.map((item) => {
            ...
            return {
              ...item,
              buy_goods_money: document.getElementById("saleSum" + i).value,
              remark: document.getElementById("remark" + i).value,
              buy_num: document.getElementById("numberSum" + i).value,
            };
          });
          ...
          $http
            .get(
              "http://localhost:5149/api/Wh_buy_order_h/Wh_buy_order_h_Update",
              {
                params: {
                  json: jsonString,
                  xx: jsonString1,
    ...
    XLKhandleSelectChange(supid) {
      supid = this.formInline.supply_code;
      if (supid == "") {
    ...
    qd() {
      ...
      try {
        const newData = this.multipleSelection1.map((item) => ({
          ...item,
          buy_goods_money: item.inbill_price,
          buy_num: 0,
        }));
    ...
    sc() {
      ...
      for (var j = 1; j <= this.tableData.length; j++) {
        ...
        document.getElementById("totalsum" + j).value = "";
      }
    ...
    find() {
      var t = this;
      $http
        .get("http://localhost:5149/api/Wh_buy_order_h/Bs_item_Find", {
          params: {
            pageSize: t.pagesize,
    ...
    findId() {
      $http
        ...
        .then((res) => {
          ...
          this.formInline.buy_date = res.buy_date;
