<template>
  <div id="kj">
    ...
    <transition-group name="lyric">
      <div id="form" v-if="showDefaultAttr" key="1">
        <el-form :inline="true" :model="formInline" class="demo-form-inline">
          <el-form-item label="进货订单" id="form_top">
            <el-input
    ...
    >
      ...
      <el-table-column fixed prop="" label="进货单号" width="150">
        <template slot-scope="scope">
          <a
            href="javascript:void(0)"
            style="color: #9c27b0"
            v-on:click="jhdh(scope.row.buy_code)"
            >{{ scope.row.buy_code }}</a
          >
        </template>
      </el-table-column>
      ...
      <el-table-column fixed label="进货日期" width="170">
        <template slot-scope="scope">
      ...
      <el-table-column prop="total_buy_num" label="货品数量" width="120">
      </el-table-column>
      <el-table-column prop="total_price" label="货品金额" width="120">
      </el-table-column>
      <el-table-column
        prop="names"
        ...
        <template slot-scope="scope">
          ...
          <span v-if="scope.row.sale_state == 1 || scope.row.sale_state == 0">
            <img
              ...
              @click="handleEdit(scope.row.buy_code)"
            />
            <img
...
export default {
  ...
  methods: {
    ...
    qd() {
      $http
        .put(
          "http://localhost:5149/api/Wh_buy_order_h/Wh_buy_order_h_sale_state?sale_state=" +
            this.resource +
            "&sure_man=" +
      ...
      if (zt == 0) {
        ...
        this.$confirm("是否撤销所选单据", "提示", {
          ...
          .then(() => {
            $http
              .put(
                "http://localhost:5149/api/Wh_buy_order_h/Wh_buy_order_h_chexiao?sure_man=" +
                  localStorage.getItem("name") +
                  "&buy_codeId=" +
                  this.buy_codeId +
                  ""
    ...
    inserok() {
      this.$router.push("/IncomingOrdersAdd");
    },
    //查询方法
    find() {
      var t = this;
    ...
    search() {
      ...
      if (this.formInline.bh != "") {
        str += "and buy_code like '%" + this.formInline.bh + "%'";
      }
    ...
    handleDelete(buy_codeId) {
      this.$confirm("此操作将永久删除该信息, 是否继续?", "提示", {
        ...
        .then(() => {
          $http
            .put(
              "http://localhost:5149/api/Wh_buy_order_h/Wh_buy_order_h_Delete?buy_codeId=" +
                buy_codeId +
                ""
            )
            .then((res) => {
    ...
    delAll() {
      this.$confirm("此操作将永久删除该信息, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          $http
            .put(
              "http://localhost:5149/api/Wh_buy_order_h/Wh_buy_order_h_DeleteArry/route",
              strs
            )
            .then((res) => {
              if (res) {
