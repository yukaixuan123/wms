    ...
    <div id="adds">
      ...
      <div style="margin-top: 30px">
        <el-form
          ...
          ref="formInline1"
        >
          <el-form-item label="出库单号" id="yy" prop="returns_code">
            <el-input
    ...
    <div id="addx">
      ...
      <div style="margin-top: 50px; margin-left: 30px">
        ...
        >
          ...
          <el-table-column label="已出库数量" width="140" v-if="kj">
            <template slot-scope="scope">
              <input
                readonly
                style="border: 0px solid; margin-left: 30px"
                v-model="scope.row.return_goods_num"
              />
            </template>
          </el-table-column>
          <el-table-column label="未出库数量" width="140" v-if="kj">
            <template slot-scope="scope">
              <!-- <span :id="'weiSum' + (scope.$index + 1)">{{
                scope.row.buy_num - tableData[scope.$index].return_goods_num
              }}</span>
              <span
              ...
              <span
                v-if="scope.row.inbill3_num - scope.row.return_goods_num > 0"
                :id="'weiSum' + (scope.$index + 1)"
                >{{ scope.row.inbill3_num - scope.row.return_goods_num }}</span
              >
              <span
                v-if="scope.row.inbill3_num - scope.row.return_goods_num < 0"
                :id="'weiSum' + (scope.$index + 1)"
...
export default {
  ...
  methods: {
    bc(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          const s = {
            create_by: localStorage.getItem("name"),
            outbill_date: this.formInline.sr_date,
            ware_code: this.formInline.ckmc,
            outbill_code: this.formInline.returns_code,
            duty_man: this.formInline.jbr,
            remark: this.formInline.bz,
            // buy_num: document.getElementById("xsSum" + j).innerHTML,
          };
          ...
          this.tableData = this.tableData.map((item) => {
            ...
            return {
              ...
              inbill_price: document.getElementById("saleSum" + i).innerHTML,
          ...
          $http
            .get(
              "http://localhost:5149/api/Wh_return_supply_h/Wh_return_supply_h_inse123r",
              {
                params: {
                  json: jsonString,
                  xx: jsonString1,
    ...
    findjb() {
      $http
        ...
        .then((res) => {
          ...
          this.formInline.cust_code = res.supply_code;
        })
        .catch((err) => {
          console.log(err);
        });
    ...
    findmx() {
      $http
        .get(
          "http://localhost:5149/api/Wh_return_supply_h/Wh_return_supply_h_t213Asyunc?returns_code=" +
            this.$route.query.returns_code +
            ""
        )
        .then((res) => {
