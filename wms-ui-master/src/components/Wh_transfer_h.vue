<template>
  <div id="kj">
    ...
    >
      ...
      <el-table-column fixed prop="outbill_code" label="调拨单号" width="150">
        <template slot-scope="{ row }">
          <router-link
            :to="{
              name: 'DetaileWh_transfer_h',
              query: { dh: row.transfer_code },
            }"
            style="color: #9c27b0"
...
export default {
  ...
  methods: {
    ...
    qd() {
      ...
      $http
        .put(
          "http://localhost:5149/api/Wh_transfer_h/Wh_transfer_hSH?name=" +
            user +
            "&zt=" +
            this.resource +
            "&desc=" +
    ...
    sh() {
      ...
      for (let i = 0; i < this.multipleSelection.length; i++) {
        ...
        var ii = this.multipleSelection[i].transfer_code;
      }
    ...
    //新增
    insert() {
      this.$router.push("/AddWh_transfer_h");
    },
    //编辑
    handleEdit(i) {
      if (i.sure_state != 0) {
      ...
      this.$router.push("/UpdateWh_transfer_h?dh=" + i.transfer_code);
    },
    ...
    FenYe() {
      ...
      $http
        .post(
          "http://localhost:5149/api/Wh_transfer_h/Fen?page=" +
            t.pagesize +
            "&pageSize=" +
            t.currentPage +
            "&str=" +
    ...
    search() {
      ...
      if (this.formInline.dh != "") {
        this.str +=
          " AND transfer_code like CONCAT('%','" +
          this.formInline.dh +
          "','%') ";
      }
      if (this.formInline.lx != "") {
        this.str +=
          " AND sure_state like CONCAT('%','" + this.formInline.lx + "','%') ";
      }
      if (this.formInline.ztai != "") {
        //入库
        if (this.formInline.ztai < 2) {
          this.str +=
            " AND inware_state like CONCAT('%','" +
            this.formInline.ztai +
            "','%') ";
        }
        //出库
        if (this.formInline.ztai > 1) {
          this.str +=
            " AND outware_state like CONCAT('%','" +
            this.formInline.ztai +
            "','%') ";
        }
      }
      if (this.formInline.mc != "" && this.formInline.mc != null) {
        const yearMonthDay = this.formInline.mc.substring(0, 10);
        this.str +=
          "and convert(varchar(50),transfer_date,120) LIKE  '" +
          yearMonthDay +
          "%'";
      }
    ...
    handleDelete(i) {
      ...
      this.$confirm("此操作将永久删除该信息, 是否继续?", "提示", {
        ...
        .then(() => {
          $http
            .put(
              "   http://localhost:5149/api/Wh_transfer_h/Wh_transfer_hDelete?addStudenDto=" +
                i.transfer_code
            )
            .then((res) => {
              if (res) {
    ...
    delAll() {
      var strs = [];
      for (let i = 0; i < this.multipleSelection.length; i++) {
        strs.push(this.multipleSelection[i].transfer_h_id);
      ...
      $http
        .put(
          " http://localhost:5149/api/Wh_transfer_h/Wh_transfer_hDeleteS/route",
          strs
        )
        .then((res) => {
          if (res) {
