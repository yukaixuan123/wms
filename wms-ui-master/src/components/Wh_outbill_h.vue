        ...
        <template slot-scope="{ row }">
          <router-link
            :to="{
              ...
              query: { dh: row.outbill_code },
            }"
            style="color: #9c27b0"
            >{{ row.outbill_code }}</router-link
          >
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    qd() {
      ...
      $http
        .put(
          "http://localhost:5149/api/Wh_outbill_h/Wh_outbill_hSH?name=" +
            user +
            "&zt=" +
            this.resource +
            "&desc=" +
      ...
      if ((this.resource = 2)) {
        $http
          .post(
            "http://localhost:5149/api/Wh_outbill_h/Wh_now_storage?dh=" +
              this.ii
          )
          .then((res) => {
            if (res) {
    ...
    handleEdit(i) {
      ...
      if (i.outbill_bh.startsWith("DB") || i.outbill_bh.startsWith("PD")) {
        this.$message({
          message: "无权限操作",
          type: "warning",
        });
    ...
    search() {
      ...
      if (this.formInline.dh != "") {
        this.str +=
          " AND outbill_code like CONCAT('%','" +
          this.formInline.dh +
          "','%') ";
      }
      if (this.formInline.lx != "") {
        this.str +=
          " AND outbill_type like CONCAT('%','" +
          this.formInline.lx +
          "','%') ";
      }
      if (this.formInline.ztai != "") {
        this.str +=
          " AND outbill_state like CONCAT('%','" +
    ...
    handleDelete(i) {
      ...
      this.$confirm("此操作将永久删除该信息, 是否继续?", "提示", {
        ...
        .then(() => {
          $http
            .put(
              "   http://localhost:5149/api/Wh_outbill_h/Wh_outbill_hDelete?addStudenDto=" +
                i.outbill_code
            )
            .then((res) => {
              if (res) {
    ...
    delAll() {
      var strs = [];
      for (let i = 0; i < this.multipleSelection.length; i++) {
        strs.push(this.multipleSelection[i].outbill_h_id);
        if (this.multipleSelection[i].outbill_state == 2) {
          this.$message({
            message: "请选择正确的记录",
            type: "warning",
          });
      ...
      $http
        .put(
          " http://localhost:5149/api/Wh_outbill_h/Wh_outbill_hDeleteS/route",
          strs
        )
        .then((res) => {
          if (res) {
