  ...
  <div>
    ...
    <div id="addsss">
      ...
      <div style="margin-top: 30px">
        ...
        >
          <el-form-item label="供应商编码" id="yyy" prop="supply_code">
            <el-input
          ...
          <el-form-item label="供应商名称" id="yyy" prop="supply_name">
            <el-input
              v-model="formInline.supply_name"
              id="tex"
              placeholder="请输入"
            ></el-input>
          </el-form-item>
          ...
          <el-form-item label="供应商分类" id="yyy">
            <el-select
              v-model="formInline.supply_type_code"
              id="tex"
              placeholder="请输入"
            >
              <el-option
                v-for="item in List2"
                :key="item.supply_type_code"
                :label="item.supply_type_code"
                :value="item.supply_type_code"
              ></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="进货负责人" id="yyy">
            <el-select
              v-model="formInline.stock_duty_man"
              id="tex"
              placeholder="请输入"
            >
              <el-option
          ...
          <el-form-item label="供应商地址" id="yyy">
            <el-input
              v-model="formInline.supply_adr"
              id="tex"
              placeholder="请输入"
            ></el-input>
          </el-form-item>
  ...
  methods: {
    ...
    bc() {
      this.$refs["ruleForm1"].validate((valid) => {
        ...
        $http
          .put(
            "http://localhost:5149/api/SupplyView/SupplyUpdate?code=" +
              t.formInline.supply_code +
              "&name=" +
              t.formInline.supply_name +
              "&type=" +
              t.formInline.line_type +
              "&typecode=" +
              t.formInline.supply_type_code +
              "&man=" +
              ...
              t.formInline.e_mail +
              "&fz=" +
              t.formInline.stock_duty_man +
              "&zt=" +
              t.radio +
              "&adr=" +
              t.formInline.supply_adr +
              "&cz=" +
              uu +
              "&bz=" +
              t.formInline.remark +
          ...
          .then((res) => {
            if (res) {
              $http
                .put(
                  "http://localhost:5149/api/SupplyView/FinanceUpdate?id=" + id,
                  {
    ...
    fl() {
      $http
        .post("http://localhost:5149/api/SupplyView/FL")
        .then((res) => {
          this.List2 = res;
          console.log(res);
        })
    ...
    fill() {
      var id = this.$route.query.id;
      $http
        .get("http://localhost:5149/api/SupplyView/SelectID?id=" + id)
        .then((res) => {
          ...
          this.radio = res.inbill_state;
          this.formInline.supply_adr = res.supply_adr;
          this.formInline.remark = res.remark;
          this.formInline1.finance_name = res.finance_name;
          this.formInline1.tax_no = res.tax_no;
          this.formInline1.bank_no = res.bank_no;
          this.formInline1.open_bank = res.open_bank;
          this.formInline1.open_adr = res.open_adr;
          console.log(res);
        })
        .catch((err) => {
          console.log(err);
